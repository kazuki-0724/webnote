      ...
      >
        <div class="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden border border-white">
          <div class="p-7">
            ...
            <p class="mt-2 text-slate-600 text-sm mb-5">保存先のフォルダと形式を選択してください。</p>
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-[0.12em] mb-2">形式</label>
                <div class="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    @click="selectedType = 'note'"
                    class="rounded-xl border px-3 py-2 text-sm font-medium transition-colors"
                    :class="selectedType === 'note' ? 'border-sky-300 bg-sky-50 text-sky-700' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'"
                  >
                    ノート
                  </button>
                  <button
                    type="button"
                    @click="selectedType = 'whiteboard'"
                    class="rounded-xl border px-3 py-2 text-sm font-medium transition-colors"
                    :class="selectedType === 'whiteboard' ? 'border-sky-300 bg-sky-50 text-sky-700' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'"
                  >
                    ホワイトボード
                  </button>
                </div>
